...
public class UserServiceImpl implements UserService {
  ...
  public String getNaverLoginAccessToken(HttpServletRequest request) {
    ...
    String clientId = "mQJ6ICVxgvpDsA5N7BtO";
    String clientSecret = "q4WqFAzCPr";
    ...
    try {
      ...
      if(responseCode == HttpURLConnection.HTTP_OK) {
        ...
        //inputStream이 byte밖에 지원안하니 streamReader로 char로 바꿔주고 거기에 buffer끼움
      } else {
      ...
      // 응답 데이터를 JSON객체로 변환하기(이때 쓰이는게 json라이브러리)
  ...
  public UserDto getNaverLoginProfile(String accessToken) {
    /************* 네이버 로그인 3 *************/
    // 네이버로부터 프로필정보(이메일, [이름, 성별, 휴대전화번호]) 을 발급 받아 반환하는 메소드
    ...
    try {
      ...
      └─ builder로 안합쳤을때. 여긴 builder가 있어서 그냥 합침
      */
      // email은 필수사항이니 꼭 들어있을 거고, 나머지 name,gender,mobile은 없을 수도 있음
      // 응답 스트림 닫기
      reader.close();
  ...
  public void signin(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      if(user != null) {
        ...
        session.setMaxInactiveInterval(1800); // 세션 유지시간 10초 설정 /* 60 * 30 : 기본값 */
